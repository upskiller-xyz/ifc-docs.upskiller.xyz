---
sidebar_position: 3
pagination_prev: ifc-daylight-factor/model-preparation
pagination_next: ifc-daylight-factor/analysis-settings
---

# Loading a Model

1. Open [dfifc.upskiller.xyz](https://dfifc.upskiller.xyz) in your browser
2. **Drag and drop** an `.ifc` file onto the drop zone, or click it to open a file picker
3. Wait for the model to load. This can take from a few seconds to a few minutes, depending on the model size.
4. _(Optional)_ Click **"Add Context"** to load additional IFC files for surrounding buildings

## Toolbar

<img src="https://upskiller-website.s3.fr-par.scw.cloud/docs.ifc/toolbar.png" alt="Scene toolbar" width="140" />

| Button       | Action                                              |
| ------------ | --------------------------------------------------- |
| Zoom Extents | Fit the whole model in view                         |
| Plan View    | Switch to a top-down view                           |
| Section Box  | Clip the model to a box to see inside               |
| Add Context  | Load additional IFC files for surrounding buildings |
| Clear All    | Remove all models and reset the scene               |

The **Analysis Settings** panel below the toolbar is covered in [Analysis Settings](./analysis-settings).

## Camera Controls

| Action      | Control                |
| ----------- | ---------------------- |
| Zoom in/out | Mouse scroll wheel     |
| Rotate view | Right-click + drag     |
| Pan view    | Middle-click + drag    |
| Reset view  | Click **Zoom Extents** |
